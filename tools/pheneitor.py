...
def parse_args():
    ...
    parser.add_argument(
        "--snptest-out", type=str, action="store", help="Destination of SNPTest input file generated by this program (defaults to /dev/null)"
    )
    parser.add_argument(
        "--gcta-out-prefix", type=str, action="store", help="Destination of GCTA phenotype file generated by this program (defaults to /dev/null)"
    )
    parser.add_argument(
...
def main(table_paths, guidetable_path, fam_path=None, subsetsamples_path=None, phenotable_output=None, phenosummary_output=sys.stdout, subset=None, gcta_dest=None, mappers=None):
    ...
    errors, counts, df, data, gtable, aliases = make_phenotable(table_paths, guidetable_path, subset=subset, samples=ss_samples, mapping=sample_id_mapping)
    ...
    # My Python installation's type system is a little clumsy so I have to manually specify summary_frames is an iterable...
    # should work without type annotations
    summary_frames: Iterable[pd.DataFrame] = [gtable, haserr.rename("has_error"), counts.add_prefix("counts:"), errors.add_prefix("error:")]
    summary_df = pd.concat(summary_frames, axis=1)

    try:
        summary_df.map(lambda x: x.replace("\n", "\\n").replace("\t", "\\t") if isinstance(x, str) else x).to_csv(phenosummary_output, sep="\t", quoting=3)
    ...
    if phenotable_output is not None:
        try:
            out_df.to_csv(phenotable_output, float_format="{:.6g}".format, sep=" ", index=False, na_rep="NA", quoting=3)
    ...
    if gcta_dest is not None:
        ...
        for pheno in phenos:
            if haserr.loc[pheno]:
                continue
            dest = lambda x: gcta_dest + "." + pheno + "." + x
            # Only binary phenotypes are saved to the .phen file
            phen = out_df[[pheno]].fillna(-9)
            try:
                phen.to_csv(dest("phen"), sep="\t", float_format="{:.6g}".format, header=False)
            except BrokenPipeError:
                pass
            ...
            covar = out_df.loc[:, (dfdt == "D") & (out_df.columns.isin(pheno_covars))].copy().fillna(-9) # TODO: is -9 a valid NA here?
            if covar.shape[1] > 0:
                try:
                    covar.to_csv(dest("covar"), sep="\t", float_format="{:.6g}".format, header=False)
                except BrokenPipeError:
                    pass

            # Only quantitative covars are saved to the .qcovar file
            qcovar = out_df.loc[:, (dfdt == "C") & (out_df.columns.isin(pheno_covars))].copy().fillna(-9) # TODO: is -9 a valid NA here?
            if qcovar.shape[1] > 0:
                try:
                    qcovar.to_csv(dest("qcovar"), sep="\t", float_format="{:.6g}".format, header=False)
                except BrokenPipeError:
                    pass
...
def report_counts(target, name, stepname, ccdata, pass_filter):
    ...
    if ccdata is not None:
        ...
        if"cases" in ccdata and ccdata["cases"] is not None:
            set_or_append_count(target, (name, f"cases:{stepname}"), (pass_filter & ccdata["cases"]).sum())
        if"controls" in ccdata and ccdata["controls"] is not None:
            set_or_append_count(target, (name, f"controls:{stepname}"), (pass_filter & ccdata["controls"]).sum())
    #else:
    #    set_or_append_count(target, (name, f"values_{stepname}"), pass_filter.sum())
    set_or_append_count(target, (name, f"values:{stepname}"), pass_filter.sum())


def make_phenotable(table_paths, guidetable_path, samples=None, subset=None, mapping={}):
    """
    ...
    # - Columns whose table (first part of the period-separated column name) is "PARTICIPANTE"
    # Aliases are convinient ways (syntactic sugar) to specify the fully-quallified
    # field name as TABLE.OTHER.PARTS (which would become PARTS)

    cnames = df.columns.to_series().map(lambda x: x.split(".")[-1])
    aliases = {}

    for _, g in cnames.groupby(cnames):
        if g.shape[0] == 1:
            aliases[g.index[0]] = g.values[0]
    
    for c, a in cnames[cnames.index.str.startswith("PARTICIPANTE.")].items():
        aliases[c] = a
    ...
    if len(cols) == 0:
        raise Exception("No phenotypes generated!")
    return (
...
def symtable(df, aliases):
    ...
    colaliases = {a: df[c] for c, a in aliases.items()}
